...
import com.face.lte_networkscan.entity.NetTimerParamEntity;
import com.face.lte_networkscan.entity.ThreadNotice;
import com.face.lte_networkscan.entity.TotalEntity;
import com.face.lte_networkscan.utils.KeyValueConst;
...
public class NetWorkListenerService extends Service {
    ...
    public void reSetAndReboot() {
        RxBus.getDefault().toObservable(String.class).subscribe(new Consumer<String>() {
            ...
            public void accept(String s) throws Exception {
                //如果重启的次数小于等于maxRebootCount
                //那么直接执行复位以及达到复位的次数后的重启操作
                if (s.equals("reset")) {
                    int rebootCount = SPUtils.getInstance().getInt(KeyValueConst.REBOOT_COUNT, 0);
                    KLog.e(TAG, "rebootCount=" + rebootCount);
                    if (rebootCount < maxRebootCount) {
                        //如果再规定的重启次数内
                        reset4G();
                        if (now4GResetCount == max4GResetCount) {
                            KLog.e(TAG, "当前已重置过4G模块" + max4GResetCount + "次,网络还是没有恢复正常，现在重启...");
                            //如果重置4G模块的次数到达三次
                            //网络还未恢复正常 则重启
                            //如果重启的次数达到一定次数
                            //保存重启的次数
                        ...
                        //否者重启的次数大于maxRebootCount
                        //判断当前的阈值是否达到要求：即如下
                        //如果recordTime能够整除30
                        int dynamicTime = SPUtils.getInstance().getInt(KeyValueConst.DYNAMIC_TIME, 0);
                        if (nowRecodeTime != 0 && nowRecodeTime == dynamicTime) {
                            KLog.e(TAG, "当前达到指定的阈值 执行重启！dynamicTime=" + dynamicTime);
                            reboot();//重启
                            return;
                        } else {
                            KLog.e(TAG, "当前未达到重启的条件 dynamicTime=" + dynamicTime + "m,当前已执行已执行：" + nowRecodeTime + "m");
                            KLog.e(TAG, "当前重启的次数为：" + rebootCount + ",最大重启的次数为：" + maxRebootCount);
                        }
                    }
    ...
    public void reboot() {
        int rebootCount = SPUtils.getInstance().getInt(KeyValueConst.REBOOT_COUNT, 0);
        SPUtils.getInstance().put(KeyValueConst.REBOOT_COUNT, rebootCount + 1);
        int dynamicTimeCardinal = SPUtils.getInstance().getInt(KeyValueConst.DYNAMIC_TIME_CARDINAL, 0);
        //将基数加1
        if ((rebootCount + 1) > maxRebootCount) {
            SPUtils.getInstance().put(KeyValueConst.DYNAMIC_TIME_CARDINAL, dynamicTimeCardinal + 1);
            SPUtils.getInstance().put(KeyValueConst.DYNAMIC_TIME, defaultRebootTime * (dynamicTimeCardinal + 1));
        } else {
            //小于等于两次的时候把设置为默认的30分钟
            SPUtils.getInstance().put(KeyValueConst.DYNAMIC_TIME_CARDINAL, 1);
            SPUtils.getInstance().put(KeyValueConst.DYNAMIC_TIME, defaultRebootTime);
        }
    ...
    public void startTimerTask1() {
        ...
        sDisposable1 = Observable
                .interval(3, cycleInterval, TimeUnit.MINUTES)
                //取消任务时取消定时唤醒
        ...
        sDisposable2 = Observable
                ...
                .subscribe(new Consumer<Long>() {
                    ...
                    public void accept(Long count) throws Exception {
                        int rebootCount = SPUtils.getInstance().getInt(KeyValueConst.REBOOT_COUNT, 0);
                        if (rebootCount >= 2) {
                            nowRecodeTime += netErrCycleTime;
                            if (!isFirstRecord) {
                                nowRecodeTime = nowRecodeTime - 1;
                                isFirstRecord = true;
                            }
                            KLog.e(TAG, "重启设备的次数大于等于" + maxRebootCount + "的时候 开始统计 nowRecodeTime=" + nowRecodeTime);
                        }
                        //总次数加1
                        int totalCount = SPUtils.getInstance().getInt(KeyValueConst.TOTAL_COUNT, 0);
                        SPUtils.getInstance().put(KeyValueConst.TOTAL_COUNT, totalCount + 1);
